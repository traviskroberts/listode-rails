class TasksController < ApplicationController
  ...
  def create
    ...
    if current_user.tasks.first(:conditions => {:title => params[:task][:title]})
      @task = current_user.tasks.first(:conditions => {:title => params[:task][:title]})
      @task.deleted_at = nil
    else
    ...
    if @task.save
      ...
      today = Date.today
      cur_list = current_user.month_lists.first(:conditions => ["year = ? AND month = ?", today.year, today.month])
      if !cur_list.nil?
      ...
      next_list = current_user.month_lists.first(:conditions => ["year = ? AND month = ?", @next_year, @next_month])
      if !next_list.nil?
